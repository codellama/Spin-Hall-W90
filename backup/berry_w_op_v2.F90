...
!                if(berry_curv_unit=='bohr2') vdum=vdum/bohr**2
    ...
    if(on_root) then
       ...
       if(eval_ahc) then                ! Output for SHC
          ...
          ! with 'V_c' in Angstroms^3, and 'e', 'hbar' in SI units
          !
          ! So we need to multiply by hbar/2*-e/hbar*1/V_c*2e/hbar
          !
...
!    enddo                       ! RF
   ...
   write(file_unit,'(10(1x,F12.6))') vel(n,m,i) !RF
        enddo
      enddo
   enddo
  ...
  !Writing out SS
    do i=1,3                            ! RF
      write(file_unit,'(/,1x,a,I2)') 'SS(nw,nw,3),Pauli matrix, component',i      !RF
        do n=1,num_wann                       ! RF
    write(file_unit,'(/)')  !RF
    write(file_unit,'(10(F12.6,1x))') SS(n,:,i) !RF
        enddo                   ! RF
    enddo                       ! RF
  ...
  ! Calculating band-resolved spin Berry curvature
   do n=1,num_wann
     do alpha=1,3 
       ...
       -2*aimag(JS(n,m,alpha,s)*vel(m,n,beta)/(eig(m)-eig(n))**2)
                endif
             enddo  
          enddo
        enddo
...
! Writing out SOmega_N
    do alpha=1,3 
      ...
      write(file_unit,'(/,1x,a,I2,I2,I2)') 'SOmega_N(nw,3,3,3),alpha, beta, s:',&  
                                alpha,beta,s      ! RF
      write(file_unit,'(10(F12.6,1x))') SOmega_N(:,alpha,beta,s) !RF
        enddo
      enddo                       ! RF
    enddo
