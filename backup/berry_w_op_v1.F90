...
module w90_berry
  ...
  public :: berry_main,get_imf_k_list,get_sbc_k_list,get_imfgh_k_list
  ...
  !============================================================!
   ...
   ! SHC calculated for a list of Fermi levels

   real(kind=dp) :: sbc_k_list(3,nfermi)  ! storing values for 'spin' Berry
   ! curvature, 3 cartesian components, at a given k point for a list of Ef values
   real(kind=dp) :: sbc_list(3,nfermi)   ! storing sum of 'spin' Berry
   ! curvature, 3 cartesian components, over the BZ for a list of Ef values
   real(kind=dp) :: shc_list(3,nfermi)  ! SHC value - 3 cartesian components
   ! for a list of Ef values (like ahc_list), 3 components are:
   ! shc(1,:) = shc_{xy}^z, shc(2,:) = shc_{yz}^x, shc(3,:) = shc_{zx}^y
   real(kind=dp) :: sbc_k_list_dummy(3,nfermi) ! adaptive refinement of SHC
   ...
   !Variables for SHC
   real(kind=dp)      :: rsdum,sdum(3)
   integer            :: s_adpt_counter_list(nfermi)     ! adaptive kmesh counter
    ...
    if(eval_ahc) then
       call get_HH_R 
       call get_AA_R                    ! We're already calling HH_R & AA_R
       imf_list=0.0_dp
       adpt_counter_list=0
       sbc_list=0.0_dp                  ! SHC
       s_adpt_counter_list=0            ! SHC
    endif
    ...
    if(on_root) then
       ...
       if(eval_ahc) write(stdout,'(/,3x,a)')&
            '* Anomalous Hall conductivity and Spin Hall Conductivity'
       ...
       do loop_xyz=1,num_int_kpts_on_node(my_node_id)
          ...
          if(eval_ahc) then
             ...
             enddo      
              ...
              ! SHC calculation
             call get_sbc_k_list(kpt,sbc_k_list)        ! output spin Berry curv
             do if=1,nfermi                             ! loop over Ef list
                sdum(1)= sbc_k_list(1,if)
                sdum(2)= sbc_k_list(2,if)
                sdum(3)= sbc_k_list(3,if)
!                if(berry_curv_unit=='bohr2') vdum=vdum/bohr**2
                rsdum=sqrt(dot_product(sdum,sdum))
                if(rsdum>berry_curv_adpt_kmesh_thresh) then     
                ! Spin BC at a kpoint above threshold turns on adaptive mesh
                   s_adpt_counter_list(if)=s_adpt_counter_list(if)+1
                   do loop_adpt=1,berry_curv_adpt_kmesh**3
                      ! Using imf_k_list here would corrupt values for other
                      ! frequencies, hence dummy. Only if-th element is used
                      call get_sbc_k_list(kpt(:)+adkpt(:,loop_adpt),&
                           sbc_k_list_dummy)
                      sbc_list(:,if)=sbc_list(:,if)&
                           +sbc_k_list_dummy(:,if)*kweight_adpt
                   end do
                else
                   sbc_list(:,if)=sbc_list(:,if)+sbc_k_list(:,if)*kweight
                endif
             enddo

          end if        ! end if(eval_ahc)
...
!                if(berry_curv_unit=='bohr2') vdum=vdum/bohr**2
    ...
    if(eval_ahc) then
       ...
       call comms_reduce(sbc_list(1,1),3*nfermi,'SUM')        ! SHC
       call comms_reduce(s_adpt_counter_list(1),nfermi,'SUM')   ! SHC
    endif
    ...
    if(on_root) then
       ...
       if(eval_ahc) then                ! Output for AHC
          !
          ! --------------------------------------------------------------------
       ...
       if(eval_ahc) then                ! Output for SHC
          !
          ! --------------------------------------------------------------------
          ! At this point sbc_list contains 
          !
          ! (1/N) sum_k Omega_{alpha beta}^s (k),
          ! which is spin Berry curvature summed over BZ, N being no. of k
          ! points in our BZ mesh 
          ! [Note: The Omega_{alpha beta}^s (k) I calculate has an extra factor 
          ! of 1/hbar on purpose - so that the adaptive mesh threshold is the same
          ! as that for Berry curvature in AHC]
          !
          ! as an approximation to 
          !
          ! V_c.int dk/(2.pi)^3 Omega_{alpha beta}^s (k) dk 
          !
          ! (V_c is the cell volume). 
          ! V_c*(V_BZ:volm of BZ) = (2pi)**3, delk ~ V_BZ/N, int dk ~ sum delk 
          !
          ! We want ISHC:
          ! sigma_{alpha beta}^s=-(e/hbar) int dk/(2.pi)^3 Omega_{alpha,beta}^s(k) dk 
          !
          ! Hence need to multiply by -(e/(hbar.V_c)). 
          ! Traditionally ISHC is given in units of Hall conductivity which
          ! involves an extra factor of 2e/hbar to get a conductivity in units of 1/(Ohm-length),
          !
          ! Assuming Berry curvature was in units of Ang^-2
          ! with 'V_c' in Angstroms^3, and 'e', 'hbar' in SI units
          !
          ! Also sbc_list has an extra factor of 1/hbar
          ! So we multiply sbc_list by fac where:
          ! ======================================
          ! fac = -2e^2/(hbar.V_c*10^-8) 
          ! ======================================
          ! In units of conductance, 
          !       (Ohm)^{-1}, or Siemens (S), to get the final result in S/cm
          !
          ! --------------------------------------------------------------------
          !
          fac=-2.0e8_dp*elem_charge_SI**2/(hbar_SI*cell_volume)
          shc_list(:,:)=sbc_list(:,:)*fac
          if(nfermi>1) then
             write(stdout,'(/,1x,a)')&
                  '---------------------------------'
             write(stdout,'(1x,a)')&
                  'Output data files related to SHC:'
             write(stdout,'(1x,a)')&
                  '---------------------------------'
             file_name=trim(seedname)//'-shc-fermiscan.dat'
                ! filename = seedname-shc-fermiscan.dat
             write(stdout,'(/,3x,a)') '* '//file_name
             file_unit=io_file_unit()
             open(file_unit,FILE=file_name,STATUS='UNKNOWN',FORM='FORMATTED')   
          endif ! Writing *fermiscan* file only if nfermi>1
          do if=1,nfermi
             if(nfermi>1) write(file_unit,'(4(F12.6,1x))')&
                  fermi_energy_list(if),shc_list(1,if),&
                  shc_list(2,if),shc_list(3,if)
             write(stdout,'(/,1x,a18,F10.4)') 'Fermi energy (ev):',&
                  fermi_energy_list(if)
             if(nfermi>1) then
                if(wanint_kpoint_file) then
                   write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                        ' Points triggering refinement: ',&
                        s_adpt_counter_list(if),'(',&
                        100*real(s_adpt_counter_list(if),dp)&
                        /sum(num_int_kpts_on_node),'%)'
                else
                   write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                        ' Points triggering refinement: ',&
             ...
             write(stdout,'(/,1x,a)')&
                  'SHC (S/cm)     Sigma_{xy}^z    Sigma_{yz}^x    Sigma_{zx}^y'
             write(stdout,'(/,1x,a)')&
                     '-------------------------------------------'
             write(stdout,'(/,1x,3(f10.4,1x),/)'),&
                     shc_list(1,if),shc_list(2,if),shc_list(3,if)
          enddo
          if(nfermi>1) close(file_unit)
       endif
  ...
  subroutine get_sbc_k_list(kpt,sbc_k_list)
  !=======================================================================!
  !                                                                       !
  ! Calculates the 'spin' Berry curvature(sbc) traced over the occupied   !
  ! states, at a given kpoint, for a list of Fermi energies               !
  ! sbc_k_list(1,if) = sbc_{x,y}^z; sbc_k_list(2,if) = sbc_{y,z}^x        !
  ! sbc_k_list(3,if) = sbc_{z,x}^y; sbc = s_omega_{alpha,beta}^s          !
  !                                                                       !
  ! s_omega_{alpha,beta}^s (sbc_k_list) =                                 ! 
  !                     sum_{n}(f_n(kpt)*s_omega_{n,alpha,beta}^s         !
  ! s_omega_{n,alpha,beta}^s (SOmega_N)  = (-2*hbar**2)*Im[sum_{m.ne.n}   !
  !              (j^s_{nm,alpha}*v_{mn,beta})/(e(m)-e(n))^2])             !
  !                                                                       !
  !j^s_{nm,alpha}=0.5sum_{l}[s_{s,nl}*v_{alpha,lm}+v_{alpha,nl}*s_{s,lm}] !
  ! j^s : spin current operator, v : vel operator shc : spin hall cond    !
  !                                                                       !
  ! shc(i,if) = -e*hcross*(1/(2pi)^3)*int_{dk}[sbc_k_list(i,if)]          !
  ! Adding an extra factor of 1/hbar to sbc_k_list to give it units of    !
  ! Berry curvature to use the same adaptive refinement kmesh             !
  !=======================================================================!

    use w90_constants, only      : dp,cmplx_0,cmplx_i,hbar_SI
    use w90_utility, only        : utility_diagonalize, utility_rotate
    use w90_parameters, only     : num_wann,nfermi,fermi_energy_list
    use w90_postw90_common, only : nrpts, fourier_R_to_k_vec, fourier_R_to_k_new,&
                                   fourier_R_to_k, get_occ
    use w90_wan_ham, only        : get_eig_deleig 
    use w90_get_oper, only       : HH_R, AA_R, get_SS_R
    ! get_HH_R and get_AA_R have already been called by berry_main if(eval_ahc) 
    ! Additionally I also have num_wann, nfermi - will not define them

    use w90_io, only             : io_error,stdout,io_file_unit,seedname        !RF
    use w90_comms, only          : on_root,num_nodes,my_node_id         !RF

    ! Arguments
    ! All arrays that are not intent in or out are allocated

    real(kind=dp), intent(in)                    :: kpt(3)
    real(kind=dp), intent(out), dimension(:,:) :: sbc_k_list

    real(kind=dp), allocatable    :: eig(:)             ! eigenvalues
    real(kind=dp), allocatable    :: del_eig(:,:)
    complex(kind=dp), allocatable :: HH(:,:)            ! Hamiltonian
    complex(kind=dp), allocatable :: delHH(:,:,:)       ! del(Ham)
    complex(kind=dp), allocatable :: delHH_bar(:,:,:)
    complex(kind=dp), allocatable :: UU(:,:)            ! Rotation matrix
    complex(kind=dp), allocatable :: AA(:,:,:)          ! Berry connection
    complex(kind=dp), allocatable :: OOmega(:,:,:)
    complex(kind=dp), allocatable :: vel(:,:,:)         ! Velocity matrix
    complex(kind=dp), allocatable :: SS_R(:,:,:,:)      ! Spin matrix
    complex(kind=dp), allocatable :: SS(:,:,:)
    complex(kind=dp), allocatable :: JS(:,:,:,:)        ! Spin current
    real(kind=dp), allocatable    :: occ(:)
    real(kind=dp), allocatable    :: SOmega_N(:,:,:,:)  ! Band-resolved spin BC

    integer                       :: i,n,m,s,alpha,beta,l,if
    character(len=24) :: file_name      ! RF
    integer           :: file_unit      ! RF

    allocate(eig(num_wann))
    allocate(del_eig(num_wann,3))
    allocate(HH(num_wann,num_wann))
    allocate(delHH(num_wann,num_wann,3))
    allocate(delHH_bar(num_wann,num_wann,3))            ! delHH in Ham gauge
    allocate(UU(num_wann,num_wann))
    allocate(AA(num_wann,num_wann,3))
    allocate(OOmega(num_wann,num_wann,3))
    allocate(vel(num_wann,num_wann,3))          ! velocity matrix in H gauge
    allocate(SS_R(num_wann,num_wann,nrpts,3))   ! Spin matrix in MLWF basis
    allocate(SS(num_wann,num_wann,3))           ! Spin matrix in H gauge
    allocate(JS(num_wann,num_wann,3,3))         ! Spin current in H gauge
    allocate(occ(num_wann))                     ! Occupation f_n(k)
    allocate(SOmega_N(num_wann,3,3,3))          ! Band resolved spin Berry
                                                ! curvature
    ...
    ! REMOVE FINALLY : RF - for output
   file_name='Shc-innerdata.dat'       !RF
   file_unit=io_file_unit()             !RF
   open(file_unit,FILE=file_name,STATUS='UNKNOWN', FORM='FORMATTED')  !RF 

   write(file_unit,'(/,1x,a)') '--------------------------------------' !RF
   write(file_unit,'(/,1x,a5,3(F12.6,1x))') 'Kpt',kpt(1),kpt(2),kpt(3) !RF
   write(file_unit,'(/,1x,a)') '--------------------------------------' !RF

   ! This will give HH and delHH, UU and eig, HH and delHH in Wannier gauge
    call get_eig_deleig(kpt,eig,del_eig,HH,delHH,UU)  

   ! This is to Fourier tranform Berry curv AA_R to give AA in Wannier gauge
    call fourier_R_to_k_vec(kpt,AA_R,OO_true=AA,OO_pseudo=OOmega)
    do i=1,3
       AA(:,:,i)=utility_rotate(AA(:,:,i),UU,num_wann)
       delHH_bar(:,:,i)=utility_rotate(delHH(:,:,i),UU,num_wann)
    enddo

    ! Writing out AA and delHH_bar 
    do i=1,3                            ! RF
      write(file_unit,'(/,1x,a,I2)') 'AA(nw,nw,3),component',i      !RF
        do n=1,num_wann                       ! RF
          write(file_unit,'(/,(F12.6,1x))') AA(n,:,i) !RF
        enddo                   ! RF
      write(file_unit,'(/,1x,a,I2)') 'delHH_bar(nw,nw,3),component',i      !RF
        do n=1,num_wann         ! RF
      write(file_unit,'(/,(F12.6,1x))') delHH_bar(n,:,i) !RF
        enddo                   ! RF
    enddo                       ! RF

   
   ! Now AA stores A^{bar,H}_alpha & delHH_bar stores H^{bar,H}_alpha
   ! Both in Hamiltonian gauge, where alpha is cartesian components 

   ! velocity matrix : v_{nm,alpha}^(H)   
   do i=1,3
   write(file_unit,'(/,1x,a,I2)') 'velocity matrix(nw,nw,3),component',i      !RF
      do n=1,num_wann
   write(file_unit,'(/)')  !RF
        do m=1,num_wann
           vel(n,m,i) = (1/hbar_SI)*delHH_bar(n,m,i) - (cmplx_i/hbar_SI)&
                        *(eig(m)-eig(n))*(AA(n,m,i))

   write(file_unit,'(2(1x,F12.6))') vel(n,m,i) !RF
        enddo
      enddo
   enddo
   ! vel(nw,nw,3) now stores velocity matrix in Hamiltonian gauge

   ! Spin matrix
   call get_SS_R

    do s=1,3
       call fourier_R_to_k(kpt,SS_R(:,:,:,s),SS(:,:,s),0)              
       SS(:,:,s)=utility_rotate(SS(:,:,s),UU,num_wann)  
    enddo
   ! SS(nw,nw,3) now stores spin matrix in Hamiltonian gauge

  !Writing out SS
    do i=1,3                            ! RF
      write(file_unit,'(/,1x,a,I2)') 'SS(nw,nw,3),Pauli matrix, component',i      !RF
        do n=1,num_wann                       ! RF
    write(file_unit,'(/,(F12.6,1x))') SS(n,:,i) !RF
!    write(file_unit,'(/,1x,num_wann(F12.6,1x),num_wann(F12.6,1x))') SS(n,:,i) !RF
        enddo                   ! RF
    enddo                       ! RF

   ! However SS_R stores the Pauli marix not spin matrix so we need a factor
   ! of hbar/2
   SS(:,:,:) = SS(:,:,:)*hbar_SI/2

  !Writing out SS
    do i=1,3                            ! RF
      write(file_unit,'(/,1x,a,I2)') 'SS,spin matrix (hbar), component',i      !RF
        do n=1,num_wann                       ! RF
    write(file_unit,'(/,(F12.6,1x))') SS(n,:,i) !RF
!    write(file_unit,'(/,1x,num_wann(F12.6,1x),num_wann(F12.6,1x))') SS(n,:,i) !RF
        enddo                   ! RF
  ...
  ! Calculating spin current operator : JS(n,m,alpha,s) = j^s_{nm,alpha}
  ! using the Spin matrix and velocity operators in H gauge


  do alpha=1,3
   do s=1,3
  write(file_unit,'(/,1x,a,I2,I2)') 'spin matrix JS(nw,nw,3,3): alpha, s:',alpha,s !RF
   do n=1,num_wann
   write(file_unit,'(/)')  !RF
     do m=1,num_wann
            do l=1,num_wann
             JS(n,m,alpha,s) = JS(n,m,alpha,s)&
            +0.5*(SS(n,l,s)*vel(l,m,alpha)+vel(n,l,alpha)*SS(l,m,s)) 
            enddo
   write(file_unit,'(2(1x,F12.6))') JS(n,m,alpha,s) !RF
          enddo
        enddo
     enddo
   enddo
 
  ! Calculating band-resolved spin Berry curvature
   do n=1,num_wann
     do alpha=1,3 
        do beta=1,3
          do s=1,3
             do m=1,num_wann
                if(m/=n) then
                     SOmega_N(n,alpha,beta,s) = SOmega_N(n,alpha,beta,s)&
       -2*(hbar_SI**2)*aimag(JS(n,m,alpha,s)*vel(m,n,beta)/(eig(m)-eig(n))**2)
                endif
             enddo  
          enddo
        enddo
...
! Writing out SOmega_N
    do alpha=1,3 
        do beta = 1,3           ! RF
          do s = 1,3           ! RF
      write(file_unit,'(/,1x,a,I2,I2,I2)') 'SOmega_N(nw,3,3,3),alpha, beta, s:',&  
                                alpha,beta,s      ! RF
      write(file_unit,'(/,(F12.6,1x))') SOmega_N(:,alpha,beta,s) !RF
!      write(file_unit,'(/,1x,num_wann(F12.6,1x))') SOmega_N(:,alpha,beta,s) !RF
        enddo
      enddo                       ! RF
    enddo
 

  ! Calculating spin Berry curvature summed over occupied states for given k 
    do if=1,nfermi
      call get_occ(eig,occ,fermi_energy_list(if))     ! get occupation
      do n=1,num_wann 
         sbc_k_list(1,if) = sbc_k_list(1,if) + occ(n)*SOmega_N(n,1,2,3)  
         sbc_k_list(2,if) = sbc_k_list(2,if) + occ(n)*SOmega_N(n,2,3,1)  
         sbc_k_list(3,if) = sbc_k_list(3,if) + occ(n)*SOmega_N(n,3,1,2)  
       enddo
    enddo

 ! Writing Spin BC
    do i=1,3                            
      write(file_unit,'(/,1x,a,I2)') 'SBC before 1/hbar, component:',i  !RF
      write(file_unit,'(/,(F12.6,1x))') sbc_k_list(i,:) !RF
!      write(file_unit,'(/,1x,nfermi(F12.6,1x))') sbc_k_list(i,:) !RF
    enddo 

   sbc_k_list(:,:) = sbc_k_list(:,:)/hbar_SI            ! Extra factor to keep
        ! values similar to Berry curvature in AHC - for adaptive refinement
        
 ! Writing Spin BC
    do i=1,3                            
      write(file_unit,'(/,1x,a,I2)') 'SBC after  1/hbar, component:',i  !RF
      write(file_unit,'(/,(F12.6,1x))') sbc_k_list(i,:) !RF
    enddo 

  close(file_unit)      !RF
 
       ! BC is vel*vel, spin BC is js*vel, js has an extra factor of hbar
  end subroutine get_sbc_k_list
