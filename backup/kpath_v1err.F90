...
contains
  ...
  subroutine k_path
    ...
    use w90_berry, only          : get_imf_k_list,get_sbc_k_list,get_imfgh_k_list
    ...
    real(kind=dp)     :: ymin,ymax,vec(3),kpt(3),spn_k(num_wann),&
                         ...
                         sbc_k_list(3,nfermi)                   !SBC
    logical           :: plot_bands,plot_curv,plot_morb
    character(len=20) :: file_name, file_name_sbc

    complex(kind=dp), allocatable :: HH(:,:)
    complex(kind=dp), allocatable :: UU(:,:)
    real(kind=dp), allocatable    :: xval(:),eig(:,:),curv(:,:),&
                                     spn_curv(:,:),&    ! stores spin berry curvature 
                                     morb(:,:),color(:,:),&
                                     plot_kpoint(:,:) 
    ...
    if(on_root) then
       ...
       if(plot_curv) then
             write(stdout,'(/,3x,a)') '* Spin Berry curvature in Ang^2'
       ...
       if(plot_curv) then
          ...
          allocate(spn_curv(total_pts,3))       ! SBC 
       endif
       ...
       do loop_kpt=1,total_pts       
          ...
          if(plot_curv) then
             ...
             call get_sbc_k_list(kpt,sbc_k_list)
!***This part-above line- seems to be the problem - where the code crashes****
          write(stdout,'(/,3x,a)') '* call get_sbc successful'        ! Debug
             spn_curv(loop_kpt,1)=sbc_k_list(1,1) ! 1st component for given Ef
          write(stdout,'(/,3x,a)') '* assigning spn_curv1 successful'        ! Debug
             spn_curv(loop_kpt,2)=sbc_k_list(2,1) ! 2nd component for given Ef
          write(stdout,'(/,3x,a)') '* assigning spn_curv2 successful'        ! Debug
             spn_curv(loop_kpt,3)=sbc_k_list(3,1) ! 3rd component for given Ef
          write(stdout,'(/,3x,a)') '* assigning spc_curv3 successful'        ! Debug
          end if                        ! nfermi = 1
       ...
       if(plot_curv) then
          ...
          dataunit=io_file_unit()               ! SBC
          file_name_sbc=trim(seedname)//'-spn_curv.dat'
          write(stdout,'(/,3x,a)') file_name_sbc
          open(dataunit,file=file_name_sbc,form='formatted')
          do loop_kpt=1,total_pts
             write(dataunit,'(4E16.8)') xval(loop_kpt),&
                  spn_curv(loop_kpt,:) 
          enddo
       ...
       if(plot_curv .and. .not.plot_bands) then
          ...
          do i=1,3
             ...
             !
             gnuunit=io_file_unit()
             file_name_sbc=trim(seedname)//'-spn_curv_'//achar(119+i)//'.gnu'
             write(stdout,'(/,3x,a)') file_name_sbc
             open(gnuunit,file=file_name_sbc,form='formatted')
             ymin=minval(spn_curv(:,i))
             ymax=maxval(spn_curv(:,i))
             range=ymax-ymin
             ymin=ymin-0.02_dp*range
             ymax=ymax+0.02_dp*range
             write(gnuunit,707) xval(total_pts),ymin,ymax
             ...
             write(gnuunit,703) glabel(1+num_paths),sum(kpath_len(:))
             write(gnuunit,*)&
                  'plot ','"'//trim(seedname)//'-spn_curv.dat','" u 1:'//achar(49+i)
             close(gnuunit)
