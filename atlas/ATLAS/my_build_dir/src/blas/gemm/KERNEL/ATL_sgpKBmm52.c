#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_sJIK0x0x52TN1x1x52_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=52, ldb=52, ldc=0, mu=1, nu=1, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   #define Mb M
   #define Nb N
   const float *stM = A + (52*(Mb));
   const float *stN = B + (52*(Nb));
   #define incAk 52
   const int incAm = 0, incAn = -(52*(Mb));
   #define incBk 52
   const int incBm = -52, incBn = 52;
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 1
   const int incCn = (ldc) - (Mb);
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0;
   register float rB0;
   register float m0;
   register float rC0_0;
   do /* N-loop */
   {
      do /* M-loop */
      {
         rA0 = beta;
         rC0_0 = *pC0;
         rC0_0 *= rA0;
/*
 *       Start pipeline
 */
         rA0 = *pA0;
         rB0 = *pB0;
         m0 = rA0 * rB0;
         rA0 = pA0[1];
         rB0 = pB0[1];

/*
 *       Completely unrolled K-loop
 */
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[2];
         rB0 = pB0[2];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[3];
         rB0 = pB0[3];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[4];
         rB0 = pB0[4];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[5];
         rB0 = pB0[5];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[6];
         rB0 = pB0[6];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[7];
         rB0 = pB0[7];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[8];
         rB0 = pB0[8];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[9];
         rB0 = pB0[9];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[10];
         rB0 = pB0[10];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[11];
         rB0 = pB0[11];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[12];
         rB0 = pB0[12];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[13];
         rB0 = pB0[13];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[14];
         rB0 = pB0[14];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[15];
         rB0 = pB0[15];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[16];
         rB0 = pB0[16];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[17];
         rB0 = pB0[17];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[18];
         rB0 = pB0[18];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[19];
         rB0 = pB0[19];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[20];
         rB0 = pB0[20];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[21];
         rB0 = pB0[21];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[22];
         rB0 = pB0[22];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[23];
         rB0 = pB0[23];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[24];
         rB0 = pB0[24];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[25];
         rB0 = pB0[25];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[26];
         rB0 = pB0[26];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[27];
         rB0 = pB0[27];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[28];
         rB0 = pB0[28];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[29];
         rB0 = pB0[29];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[30];
         rB0 = pB0[30];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[31];
         rB0 = pB0[31];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[32];
         rB0 = pB0[32];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[33];
         rB0 = pB0[33];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[34];
         rB0 = pB0[34];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[35];
         rB0 = pB0[35];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[36];
         rB0 = pB0[36];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[37];
         rB0 = pB0[37];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[38];
         rB0 = pB0[38];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[39];
         rB0 = pB0[39];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[40];
         rB0 = pB0[40];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[41];
         rB0 = pB0[41];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[42];
         rB0 = pB0[42];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[43];
         rB0 = pB0[43];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[44];
         rB0 = pB0[44];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[45];
         rB0 = pB0[45];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[46];
         rB0 = pB0[46];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[47];
         rB0 = pB0[47];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[48];
         rB0 = pB0[48];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[49];
         rB0 = pB0[49];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[50];
         rB0 = pB0[50];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[51];
         rB0 = pB0[51];
/*
 *       Drain pipe on last iteration of K-loop
 */
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rC0_0 += m0;
         pA0 += incAk;
         pB0 += incBk;
         *pC0 = rC0_0;
         pC0 += incCm;
         pA0 += incAm;
         pB0 += incBm;
      }
      while(pA0 != stM);
      pC0 += incCn;
      pA0 += incAn;
      pB0 += incBn;
   }
   while(pB0 != stN);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
static void ATL_sJIK0x0x52TN2x1x52_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=52, ldb=52, ldc=0, mu=2, nu=1, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   const int Mb = (M>>1)<<1;
   #define Nb N
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + (52*(Mb));
   const float *stN = B + (52*(Nb));
   #define incAk 52
   const int incAm = 52, incAn = -(52*(Mb));
   #define incBk 52
   const int incBm = -52, incBn = 52;
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 2
   const int incCn = (ldc) - (Mb);
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0, rA1;
   register float rB0;
   register float m0;
   register float rC0_0, rC1_0;
   if (pA0 != stM)
   {
      do /* N-loop */
      {
         do /* M-loop */
         {
            rA0 = beta;
            rC0_0 = *pC0;
            rC0_0 *= rA0;
            rC1_0 = pC0[1];
            rC1_0 *= rA0;
/*
 *          Start pipeline
 */
            rA0 = *pA0;
            rB0 = *pB0;
            rA1 = pA0[52];
            m0 = rA0 * rB0;

/*
 *          Completely unrolled K-loop
 */
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[1];
            rB0 = pB0[1];
            rA1 = pA0[53];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[2];
            rB0 = pB0[2];
            rA1 = pA0[54];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[3];
            rB0 = pB0[3];
            rA1 = pA0[55];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[4];
            rB0 = pB0[4];
            rA1 = pA0[56];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[5];
            rB0 = pB0[5];
            rA1 = pA0[57];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[6];
            rB0 = pB0[6];
            rA1 = pA0[58];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[7];
            rB0 = pB0[7];
            rA1 = pA0[59];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[8];
            rB0 = pB0[8];
            rA1 = pA0[60];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[9];
            rB0 = pB0[9];
            rA1 = pA0[61];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[10];
            rB0 = pB0[10];
            rA1 = pA0[62];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[11];
            rB0 = pB0[11];
            rA1 = pA0[63];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[12];
            rB0 = pB0[12];
            rA1 = pA0[64];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[13];
            rB0 = pB0[13];
            rA1 = pA0[65];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[14];
            rB0 = pB0[14];
            rA1 = pA0[66];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[15];
            rB0 = pB0[15];
            rA1 = pA0[67];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[16];
            rB0 = pB0[16];
            rA1 = pA0[68];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[17];
            rB0 = pB0[17];
            rA1 = pA0[69];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[18];
            rB0 = pB0[18];
            rA1 = pA0[70];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[19];
            rB0 = pB0[19];
            rA1 = pA0[71];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[20];
            rB0 = pB0[20];
            rA1 = pA0[72];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[21];
            rB0 = pB0[21];
            rA1 = pA0[73];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[22];
            rB0 = pB0[22];
            rA1 = pA0[74];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[23];
            rB0 = pB0[23];
            rA1 = pA0[75];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[24];
            rB0 = pB0[24];
            rA1 = pA0[76];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[25];
            rB0 = pB0[25];
            rA1 = pA0[77];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[26];
            rB0 = pB0[26];
            rA1 = pA0[78];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[27];
            rB0 = pB0[27];
            rA1 = pA0[79];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[28];
            rB0 = pB0[28];
            rA1 = pA0[80];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[29];
            rB0 = pB0[29];
            rA1 = pA0[81];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[30];
            rB0 = pB0[30];
            rA1 = pA0[82];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[31];
            rB0 = pB0[31];
            rA1 = pA0[83];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[32];
            rB0 = pB0[32];
            rA1 = pA0[84];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[33];
            rB0 = pB0[33];
            rA1 = pA0[85];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[34];
            rB0 = pB0[34];
            rA1 = pA0[86];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[35];
            rB0 = pB0[35];
            rA1 = pA0[87];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[36];
            rB0 = pB0[36];
            rA1 = pA0[88];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[37];
            rB0 = pB0[37];
            rA1 = pA0[89];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[38];
            rB0 = pB0[38];
            rA1 = pA0[90];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[39];
            rB0 = pB0[39];
            rA1 = pA0[91];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[40];
            rB0 = pB0[40];
            rA1 = pA0[92];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[41];
            rB0 = pB0[41];
            rA1 = pA0[93];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[42];
            rB0 = pB0[42];
            rA1 = pA0[94];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[43];
            rB0 = pB0[43];
            rA1 = pA0[95];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[44];
            rB0 = pB0[44];
            rA1 = pA0[96];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[45];
            rB0 = pB0[45];
            rA1 = pA0[97];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[46];
            rB0 = pB0[46];
            rA1 = pA0[98];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[47];
            rB0 = pB0[47];
            rA1 = pA0[99];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[48];
            rB0 = pB0[48];
            rA1 = pA0[100];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[49];
            rB0 = pB0[49];
            rA1 = pA0[101];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[50];
            rB0 = pB0[50];
            rA1 = pA0[102];
            rC1_0 += m0;
            m0 = rA0 * rB0;
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rA0 = pA0[51];
            rB0 = pB0[51];
            rA1 = pA0[103];
            rC1_0 += m0;
            m0 = rA0 * rB0;
/*
 *          Drain pipe on last iteration of K-loop
 */
            rC0_0 += m0;
            m0 = rA1 * rB0;
            rC1_0 += m0;
            pA0 += incAk;
            pB0 += incBk;
            *pC0 = rC0_0;
            pC0[1] = rC1_0;
            pC0 += incCm;
            pA0 += incAm;
            pB0 += incBm;
         }
         while(pA0 != stM);
         pC0 += incCn;
         pA0 += incAn;
         pB0 += incBn;
      }
      while(pB0 != stN);
   }
   if (k=M-Mb)
      ATL_sJIK0x0x52TN1x1x52_a1_bX(k, N, 52, alpha, ca + (52*(Mb)), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_sJIK0x0x52TN1x5x52_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=52, ldb=52, ldc=0, mu=1, nu=5, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   #define Mb M
   const int Nb = (N/5)*5;
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + (52*(Mb));
   const float *stN = B + (52*(Nb));
   #define incAk 52
   const int incAm = 0, incAn = -(52*(Mb));
   #define incBk 52
   const int incBm = -52, incBn = 260;
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 1
   const int incCn = (((ldc) << 2)+ldc) - (Mb);
   float *pC0=C, *pC1=pC0+(ldc), *pC2=pC1+(ldc), *pC3=pC2+(ldc), *pC4=pC3+(ldc);
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0;
   register float rB0, rB1, rB2, rB3, rB4;
   register float m0;
   register float rC0_0, rC0_1, rC0_2, rC0_3, rC0_4;
   if (pB0 != stN)
   {
      do /* N-loop */
      {
         do /* M-loop */
         {
            rA0 = beta;
            rC0_0 = *pC0;
            rC0_0 *= rA0;
            rC0_1 = *pC1;
            rC0_1 *= rA0;
            rC0_2 = *pC2;
            rC0_2 *= rA0;
            rC0_3 = *pC3;
            rC0_3 *= rA0;
            rC0_4 = *pC4;
            rC0_4 *= rA0;
/*
 *          Start pipeline
 */
            rA0 = *pA0;
            rB0 = *pB0;
            rB1 = pB0[52];
            m0 = rA0 * rB0;
            rB2 = pB0[104];
            rB3 = pB0[156];

/*
 *          Completely unrolled K-loop
 */
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rB4 = pB0[208];
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[1];
            rB0 = pB0[1];
            rB1 = pB0[53];
            rB2 = pB0[105];
            rB3 = pB0[157];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[209];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[2];
            rB0 = pB0[2];
            rB1 = pB0[54];
            rB2 = pB0[106];
            rB3 = pB0[158];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[210];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[3];
            rB0 = pB0[3];
            rB1 = pB0[55];
            rB2 = pB0[107];
            rB3 = pB0[159];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[211];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[4];
            rB0 = pB0[4];
            rB1 = pB0[56];
            rB2 = pB0[108];
            rB3 = pB0[160];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[212];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[5];
            rB0 = pB0[5];
            rB1 = pB0[57];
            rB2 = pB0[109];
            rB3 = pB0[161];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[213];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[6];
            rB0 = pB0[6];
            rB1 = pB0[58];
            rB2 = pB0[110];
            rB3 = pB0[162];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[214];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[7];
            rB0 = pB0[7];
            rB1 = pB0[59];
            rB2 = pB0[111];
            rB3 = pB0[163];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[215];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[8];
            rB0 = pB0[8];
            rB1 = pB0[60];
            rB2 = pB0[112];
            rB3 = pB0[164];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[216];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[9];
            rB0 = pB0[9];
            rB1 = pB0[61];
            rB2 = pB0[113];
            rB3 = pB0[165];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[217];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[10];
            rB0 = pB0[10];
            rB1 = pB0[62];
            rB2 = pB0[114];
            rB3 = pB0[166];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[218];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[11];
            rB0 = pB0[11];
            rB1 = pB0[63];
            rB2 = pB0[115];
            rB3 = pB0[167];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[219];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[12];
            rB0 = pB0[12];
            rB1 = pB0[64];
            rB2 = pB0[116];
            rB3 = pB0[168];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[220];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[13];
            rB0 = pB0[13];
            rB1 = pB0[65];
            rB2 = pB0[117];
            rB3 = pB0[169];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[221];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[14];
            rB0 = pB0[14];
            rB1 = pB0[66];
            rB2 = pB0[118];
            rB3 = pB0[170];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[222];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[15];
            rB0 = pB0[15];
            rB1 = pB0[67];
            rB2 = pB0[119];
            rB3 = pB0[171];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[223];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[16];
            rB0 = pB0[16];
            rB1 = pB0[68];
            rB2 = pB0[120];
            rB3 = pB0[172];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[224];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[17];
            rB0 = pB0[17];
            rB1 = pB0[69];
            rB2 = pB0[121];
            rB3 = pB0[173];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[225];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[18];
            rB0 = pB0[18];
            rB1 = pB0[70];
            rB2 = pB0[122];
            rB3 = pB0[174];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[226];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[19];
            rB0 = pB0[19];
            rB1 = pB0[71];
            rB2 = pB0[123];
            rB3 = pB0[175];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[227];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[20];
            rB0 = pB0[20];
            rB1 = pB0[72];
            rB2 = pB0[124];
            rB3 = pB0[176];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[228];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[21];
            rB0 = pB0[21];
            rB1 = pB0[73];
            rB2 = pB0[125];
            rB3 = pB0[177];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[229];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[22];
            rB0 = pB0[22];
            rB1 = pB0[74];
            rB2 = pB0[126];
            rB3 = pB0[178];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[230];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[23];
            rB0 = pB0[23];
            rB1 = pB0[75];
            rB2 = pB0[127];
            rB3 = pB0[179];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[231];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[24];
            rB0 = pB0[24];
            rB1 = pB0[76];
            rB2 = pB0[128];
            rB3 = pB0[180];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[232];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[25];
            rB0 = pB0[25];
            rB1 = pB0[77];
            rB2 = pB0[129];
            rB3 = pB0[181];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[233];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[26];
            rB0 = pB0[26];
            rB1 = pB0[78];
            rB2 = pB0[130];
            rB3 = pB0[182];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[234];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[27];
            rB0 = pB0[27];
            rB1 = pB0[79];
            rB2 = pB0[131];
            rB3 = pB0[183];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[235];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[28];
            rB0 = pB0[28];
            rB1 = pB0[80];
            rB2 = pB0[132];
            rB3 = pB0[184];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[236];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[29];
            rB0 = pB0[29];
            rB1 = pB0[81];
            rB2 = pB0[133];
            rB3 = pB0[185];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[237];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[30];
            rB0 = pB0[30];
            rB1 = pB0[82];
            rB2 = pB0[134];
            rB3 = pB0[186];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[238];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[31];
            rB0 = pB0[31];
            rB1 = pB0[83];
            rB2 = pB0[135];
            rB3 = pB0[187];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[239];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[32];
            rB0 = pB0[32];
            rB1 = pB0[84];
            rB2 = pB0[136];
            rB3 = pB0[188];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[240];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[33];
            rB0 = pB0[33];
            rB1 = pB0[85];
            rB2 = pB0[137];
            rB3 = pB0[189];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[241];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[34];
            rB0 = pB0[34];
            rB1 = pB0[86];
            rB2 = pB0[138];
            rB3 = pB0[190];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[242];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[35];
            rB0 = pB0[35];
            rB1 = pB0[87];
            rB2 = pB0[139];
            rB3 = pB0[191];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[243];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[36];
            rB0 = pB0[36];
            rB1 = pB0[88];
            rB2 = pB0[140];
            rB3 = pB0[192];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[244];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[37];
            rB0 = pB0[37];
            rB1 = pB0[89];
            rB2 = pB0[141];
            rB3 = pB0[193];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[245];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[38];
            rB0 = pB0[38];
            rB1 = pB0[90];
            rB2 = pB0[142];
            rB3 = pB0[194];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[246];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[39];
            rB0 = pB0[39];
            rB1 = pB0[91];
            rB2 = pB0[143];
            rB3 = pB0[195];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[247];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[40];
            rB0 = pB0[40];
            rB1 = pB0[92];
            rB2 = pB0[144];
            rB3 = pB0[196];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[248];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[41];
            rB0 = pB0[41];
            rB1 = pB0[93];
            rB2 = pB0[145];
            rB3 = pB0[197];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[249];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[42];
            rB0 = pB0[42];
            rB1 = pB0[94];
            rB2 = pB0[146];
            rB3 = pB0[198];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[250];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[43];
            rB0 = pB0[43];
            rB1 = pB0[95];
            rB2 = pB0[147];
            rB3 = pB0[199];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[251];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[44];
            rB0 = pB0[44];
            rB1 = pB0[96];
            rB2 = pB0[148];
            rB3 = pB0[200];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[252];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[45];
            rB0 = pB0[45];
            rB1 = pB0[97];
            rB2 = pB0[149];
            rB3 = pB0[201];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[253];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[46];
            rB0 = pB0[46];
            rB1 = pB0[98];
            rB2 = pB0[150];
            rB3 = pB0[202];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[254];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[47];
            rB0 = pB0[47];
            rB1 = pB0[99];
            rB2 = pB0[151];
            rB3 = pB0[203];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[255];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[48];
            rB0 = pB0[48];
            rB1 = pB0[100];
            rB2 = pB0[152];
            rB3 = pB0[204];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[256];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[49];
            rB0 = pB0[49];
            rB1 = pB0[101];
            rB2 = pB0[153];
            rB3 = pB0[205];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[257];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[50];
            rB0 = pB0[50];
            rB1 = pB0[102];
            rB2 = pB0[154];
            rB3 = pB0[206];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[258];
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rA0 = pA0[51];
            rB0 = pB0[51];
            rB1 = pB0[103];
            rB2 = pB0[155];
            rB3 = pB0[207];
            rC0_4 += m0;
            m0 = rA0 * rB0;
            rB4 = pB0[259];
/*
 *          Drain pipe on last iteration of K-loop
 */
            rC0_0 += m0;
            m0 = rA0 * rB1;
            rC0_1 += m0;
            m0 = rA0 * rB2;
            rC0_2 += m0;
            m0 = rA0 * rB3;
            rC0_3 += m0;
            m0 = rA0 * rB4;
            rC0_4 += m0;
            pA0 += incAk;
            pB0 += incBk;
            *pC0 = rC0_0;
            *pC1 = rC0_1;
            *pC2 = rC0_2;
            *pC3 = rC0_3;
            *pC4 = rC0_4;
            pC0 += incCm;
            pC1 += incCm;
            pC2 += incCm;
            pC3 += incCm;
            pC4 += incCm;
            pA0 += incAm;
            pB0 += incBm;
         }
         while(pA0 != stM);
         pC0 += incCn;
         pC1 += incCn;
         pC2 += incCn;
         pC3 += incCn;
         pC4 += incCn;
         pA0 += incAn;
         pB0 += incBn;
      }
      while(pB0 != stN);
   }
   if (k=N-Nb)
      ATL_sJIK0x0x52TN1x1x52_a1_bX(M, k, 52, alpha, ca, lda, cb + (52*(Nb)), ldb, beta, cc + (Nb*ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
void ATL_sJIK0x0x52TN52x52x0_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=52, ldb=52, ldc=0, mu=2, nu=5, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   const int Mb = (M>>1)<<1;
   const int Nb = (N/5)*5;
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + (52*(Mb));
   const float *stN = B + (52*(Nb));
   #define incAk 52
   const int incAm = 52, incAn = -(52*(Mb));
   #define incBk 52
   const int incBm = -52, incBn = 260;
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 2
   const int incCn = (((ldc) << 2)+ldc) - (Mb);
   float *pC0=C, *pC1=pC0+(ldc), *pC2=pC1+(ldc), *pC3=pC2+(ldc), *pC4=pC3+(ldc);
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0, rA1;
   register float rB0, rB1, rB2, rB3, rB4;
   register float m0;
   register float rC0_0, rC1_0, rC0_1, rC1_1, rC0_2, rC1_2, rC0_3, rC1_3, rC0_4, rC1_4;
   if (pA0 != stM)
   {
      if (pB0 != stN)
      {
         do /* N-loop */
         {
            do /* M-loop */
            {
               rA0 = beta;
               rC0_0 = *pC0;
               rC0_0 *= rA0;
               rC1_0 = pC0[1];
               rC1_0 *= rA0;
               rC0_1 = *pC1;
               rC0_1 *= rA0;
               rC1_1 = pC1[1];
               rC1_1 *= rA0;
               rC0_2 = *pC2;
               rC0_2 *= rA0;
               rC1_2 = pC2[1];
               rC1_2 *= rA0;
               rC0_3 = *pC3;
               rC0_3 *= rA0;
               rC1_3 = pC3[1];
               rC1_3 *= rA0;
               rC0_4 = *pC4;
               rC0_4 *= rA0;
               rC1_4 = pC4[1];
               rC1_4 *= rA0;
/*
 *             Start pipeline
 */
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[52];
               m0 = rA0 * rB0;
               rB1 = pB0[52];
               rB2 = pB0[104];

/*
 *             Completely unrolled K-loop
 */
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rB3 = pB0[156];
               rB4 = pB0[208];
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[1];
               rB0 = pB0[1];
               rA1 = pA0[53];
               rB1 = pB0[53];
               rB2 = pB0[105];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[157];
               rB4 = pB0[209];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[2];
               rB0 = pB0[2];
               rA1 = pA0[54];
               rB1 = pB0[54];
               rB2 = pB0[106];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[158];
               rB4 = pB0[210];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[3];
               rB0 = pB0[3];
               rA1 = pA0[55];
               rB1 = pB0[55];
               rB2 = pB0[107];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[159];
               rB4 = pB0[211];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[4];
               rB0 = pB0[4];
               rA1 = pA0[56];
               rB1 = pB0[56];
               rB2 = pB0[108];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[160];
               rB4 = pB0[212];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[5];
               rB0 = pB0[5];
               rA1 = pA0[57];
               rB1 = pB0[57];
               rB2 = pB0[109];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[161];
               rB4 = pB0[213];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[6];
               rB0 = pB0[6];
               rA1 = pA0[58];
               rB1 = pB0[58];
               rB2 = pB0[110];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[162];
               rB4 = pB0[214];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[7];
               rB0 = pB0[7];
               rA1 = pA0[59];
               rB1 = pB0[59];
               rB2 = pB0[111];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[163];
               rB4 = pB0[215];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[8];
               rB0 = pB0[8];
               rA1 = pA0[60];
               rB1 = pB0[60];
               rB2 = pB0[112];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[164];
               rB4 = pB0[216];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[9];
               rB0 = pB0[9];
               rA1 = pA0[61];
               rB1 = pB0[61];
               rB2 = pB0[113];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[165];
               rB4 = pB0[217];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[10];
               rB0 = pB0[10];
               rA1 = pA0[62];
               rB1 = pB0[62];
               rB2 = pB0[114];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[166];
               rB4 = pB0[218];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[11];
               rB0 = pB0[11];
               rA1 = pA0[63];
               rB1 = pB0[63];
               rB2 = pB0[115];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[167];
               rB4 = pB0[219];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[12];
               rB0 = pB0[12];
               rA1 = pA0[64];
               rB1 = pB0[64];
               rB2 = pB0[116];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[168];
               rB4 = pB0[220];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[13];
               rB0 = pB0[13];
               rA1 = pA0[65];
               rB1 = pB0[65];
               rB2 = pB0[117];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[169];
               rB4 = pB0[221];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[14];
               rB0 = pB0[14];
               rA1 = pA0[66];
               rB1 = pB0[66];
               rB2 = pB0[118];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[170];
               rB4 = pB0[222];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[15];
               rB0 = pB0[15];
               rA1 = pA0[67];
               rB1 = pB0[67];
               rB2 = pB0[119];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[171];
               rB4 = pB0[223];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[16];
               rB0 = pB0[16];
               rA1 = pA0[68];
               rB1 = pB0[68];
               rB2 = pB0[120];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[172];
               rB4 = pB0[224];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[17];
               rB0 = pB0[17];
               rA1 = pA0[69];
               rB1 = pB0[69];
               rB2 = pB0[121];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[173];
               rB4 = pB0[225];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[18];
               rB0 = pB0[18];
               rA1 = pA0[70];
               rB1 = pB0[70];
               rB2 = pB0[122];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[174];
               rB4 = pB0[226];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[19];
               rB0 = pB0[19];
               rA1 = pA0[71];
               rB1 = pB0[71];
               rB2 = pB0[123];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[175];
               rB4 = pB0[227];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[20];
               rB0 = pB0[20];
               rA1 = pA0[72];
               rB1 = pB0[72];
               rB2 = pB0[124];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[176];
               rB4 = pB0[228];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[21];
               rB0 = pB0[21];
               rA1 = pA0[73];
               rB1 = pB0[73];
               rB2 = pB0[125];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[177];
               rB4 = pB0[229];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[22];
               rB0 = pB0[22];
               rA1 = pA0[74];
               rB1 = pB0[74];
               rB2 = pB0[126];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[178];
               rB4 = pB0[230];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[23];
               rB0 = pB0[23];
               rA1 = pA0[75];
               rB1 = pB0[75];
               rB2 = pB0[127];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[179];
               rB4 = pB0[231];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[24];
               rB0 = pB0[24];
               rA1 = pA0[76];
               rB1 = pB0[76];
               rB2 = pB0[128];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[180];
               rB4 = pB0[232];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[25];
               rB0 = pB0[25];
               rA1 = pA0[77];
               rB1 = pB0[77];
               rB2 = pB0[129];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[181];
               rB4 = pB0[233];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[26];
               rB0 = pB0[26];
               rA1 = pA0[78];
               rB1 = pB0[78];
               rB2 = pB0[130];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[182];
               rB4 = pB0[234];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[27];
               rB0 = pB0[27];
               rA1 = pA0[79];
               rB1 = pB0[79];
               rB2 = pB0[131];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[183];
               rB4 = pB0[235];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[28];
               rB0 = pB0[28];
               rA1 = pA0[80];
               rB1 = pB0[80];
               rB2 = pB0[132];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[184];
               rB4 = pB0[236];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[29];
               rB0 = pB0[29];
               rA1 = pA0[81];
               rB1 = pB0[81];
               rB2 = pB0[133];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[185];
               rB4 = pB0[237];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[30];
               rB0 = pB0[30];
               rA1 = pA0[82];
               rB1 = pB0[82];
               rB2 = pB0[134];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[186];
               rB4 = pB0[238];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[31];
               rB0 = pB0[31];
               rA1 = pA0[83];
               rB1 = pB0[83];
               rB2 = pB0[135];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[187];
               rB4 = pB0[239];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[32];
               rB0 = pB0[32];
               rA1 = pA0[84];
               rB1 = pB0[84];
               rB2 = pB0[136];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[188];
               rB4 = pB0[240];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[33];
               rB0 = pB0[33];
               rA1 = pA0[85];
               rB1 = pB0[85];
               rB2 = pB0[137];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[189];
               rB4 = pB0[241];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[34];
               rB0 = pB0[34];
               rA1 = pA0[86];
               rB1 = pB0[86];
               rB2 = pB0[138];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[190];
               rB4 = pB0[242];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[35];
               rB0 = pB0[35];
               rA1 = pA0[87];
               rB1 = pB0[87];
               rB2 = pB0[139];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[191];
               rB4 = pB0[243];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[36];
               rB0 = pB0[36];
               rA1 = pA0[88];
               rB1 = pB0[88];
               rB2 = pB0[140];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[192];
               rB4 = pB0[244];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[37];
               rB0 = pB0[37];
               rA1 = pA0[89];
               rB1 = pB0[89];
               rB2 = pB0[141];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[193];
               rB4 = pB0[245];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[38];
               rB0 = pB0[38];
               rA1 = pA0[90];
               rB1 = pB0[90];
               rB2 = pB0[142];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[194];
               rB4 = pB0[246];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[39];
               rB0 = pB0[39];
               rA1 = pA0[91];
               rB1 = pB0[91];
               rB2 = pB0[143];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[195];
               rB4 = pB0[247];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[40];
               rB0 = pB0[40];
               rA1 = pA0[92];
               rB1 = pB0[92];
               rB2 = pB0[144];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[196];
               rB4 = pB0[248];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[41];
               rB0 = pB0[41];
               rA1 = pA0[93];
               rB1 = pB0[93];
               rB2 = pB0[145];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[197];
               rB4 = pB0[249];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[42];
               rB0 = pB0[42];
               rA1 = pA0[94];
               rB1 = pB0[94];
               rB2 = pB0[146];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[198];
               rB4 = pB0[250];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[43];
               rB0 = pB0[43];
               rA1 = pA0[95];
               rB1 = pB0[95];
               rB2 = pB0[147];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[199];
               rB4 = pB0[251];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[44];
               rB0 = pB0[44];
               rA1 = pA0[96];
               rB1 = pB0[96];
               rB2 = pB0[148];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[200];
               rB4 = pB0[252];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[45];
               rB0 = pB0[45];
               rA1 = pA0[97];
               rB1 = pB0[97];
               rB2 = pB0[149];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[201];
               rB4 = pB0[253];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[46];
               rB0 = pB0[46];
               rA1 = pA0[98];
               rB1 = pB0[98];
               rB2 = pB0[150];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[202];
               rB4 = pB0[254];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[47];
               rB0 = pB0[47];
               rA1 = pA0[99];
               rB1 = pB0[99];
               rB2 = pB0[151];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[203];
               rB4 = pB0[255];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[48];
               rB0 = pB0[48];
               rA1 = pA0[100];
               rB1 = pB0[100];
               rB2 = pB0[152];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[204];
               rB4 = pB0[256];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[49];
               rB0 = pB0[49];
               rA1 = pA0[101];
               rB1 = pB0[101];
               rB2 = pB0[153];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[205];
               rB4 = pB0[257];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[50];
               rB0 = pB0[50];
               rA1 = pA0[102];
               rB1 = pB0[102];
               rB2 = pB0[154];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[206];
               rB4 = pB0[258];
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rA0 = pA0[51];
               rB0 = pB0[51];
               rA1 = pA0[103];
               rB1 = pB0[103];
               rB2 = pB0[155];
               rC1_4 += m0;
               m0 = rA0 * rB0;
               rB3 = pB0[207];
               rB4 = pB0[259];
/*
 *             Drain pipe on last iteration of K-loop
 */
               rC0_0 += m0;
               m0 = rA1 * rB0;
               rC1_0 += m0;
               m0 = rA0 * rB1;
               rC0_1 += m0;
               m0 = rA1 * rB1;
               rC1_1 += m0;
               m0 = rA0 * rB2;
               rC0_2 += m0;
               m0 = rA1 * rB2;
               rC1_2 += m0;
               m0 = rA0 * rB3;
               rC0_3 += m0;
               m0 = rA1 * rB3;
               rC1_3 += m0;
               m0 = rA0 * rB4;
               rC0_4 += m0;
               m0 = rA1 * rB4;
               rC1_4 += m0;
               pA0 += incAk;
               pB0 += incBk;
               *pC0 = rC0_0;
               pC0[1] = rC1_0;
               *pC1 = rC0_1;
               pC1[1] = rC1_1;
               *pC2 = rC0_2;
               pC2[1] = rC1_2;
               *pC3 = rC0_3;
               pC3[1] = rC1_3;
               *pC4 = rC0_4;
               pC4[1] = rC1_4;
               pC0 += incCm;
               pC1 += incCm;
               pC2 += incCm;
               pC3 += incCm;
               pC4 += incCm;
               pA0 += incAm;
               pB0 += incBm;
            }
            while(pA0 != stM);
            pC0 += incCn;
            pC1 += incCn;
            pC2 += incCn;
            pC3 += incCn;
            pC4 += incCn;
            pA0 += incAn;
            pB0 += incBn;
         }
         while(pB0 != stN);
      }
   }
   if (k=N-Nb)
      ATL_sJIK0x0x52TN2x1x52_a1_bX(M, k, 52, alpha, ca, lda, cb + (52*(Nb)), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_sJIK0x0x52TN1x5x52_a1_bX(k, Nb, 52, alpha, ca + (52*(Mb)), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
