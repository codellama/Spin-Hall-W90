...
static void ATL_dJIK0x0x40TN1x1x40_aX_bX
 ...
 */
{
   ...
   #define incAk 40
   const int incAm = ((lda) - 40), incAn = -(Mb*lda);
   #define incBk 40
 ...
 */
   ...
   #undef Kb
#endif
void ATL_dJIK0x0x40TN0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=40, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   #define incAk 40
   const int incAm = ((((lda) << 3)+((lda) << 1)) - 40), incAn = -(Mb*lda);
   #define incBk 40
   const int incBm = -40, incBn = (ldb);
   const int incAk0 = ((incAk) / 40), incBk0 = ((incBk) / 40);
   #define incCm 10
 ...
 */
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x40TN1x1x40_aX_bX(k, N, 40, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
