...
static void ATL_zJIK0x0x40TN1x1x40_aX_bX
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_zJIK0x0x40TN0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=40, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   #define incAk 80
   const int incAm = ((((((lda) << 3)+((lda) << 1)) - 40) << 1)), incAn = -(((Mb*lda) << 1));
   #define incBk 80
   const int incBm = -80, incBn = (((ldb) << 1));
   #define incCm 20
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   ...
   if (k=M-Mb)
      ATL_zJIK0x0x40TN1x1x40_aX_bX(k, N, 40, alpha, ca + (((Mb*lda) << 1)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
