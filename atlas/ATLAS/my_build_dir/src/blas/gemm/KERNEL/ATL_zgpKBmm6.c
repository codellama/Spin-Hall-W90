...
static void ATL_zJIK0x0x6TN1x1x6_a1_bX
 ...
 */
{
   #define Mb M
   #define Nb N
   const double *stM = A + (((Mb) << 2)+((Mb) << 1));
   const double *stN = B + (((Nb) << 2)+((Nb) << 1));
   ...
   #undef Kb
#endif
void ATL_zJIK0x0x6TN6x6x0_a1_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=6, 
 * lda=6, ldb=6, ldc=0, mu=10, nu=1, ku=6, pf=1
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   ...
   const int incPFA = (1 > incPFA0) ? 1 : incPFA0;
   #define incAk 6
   const int incAm = 54, incAn = -(((Mb) << 2)+((Mb) << 1));
   #define incBk 6
   const int incBm = -6, incBn = 6;
   #define incCm 20
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   ...
   if (k=M-Mb)
      ATL_zJIK0x0x6TN1x1x6_a1_bX(k, N, 6, alpha, ca + (((Mb) << 2)+((Mb) << 1)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
