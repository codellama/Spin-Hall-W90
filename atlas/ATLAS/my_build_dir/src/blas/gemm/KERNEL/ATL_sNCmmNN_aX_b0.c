#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_sJIK68x3x68NN2x3x68_aX_b0
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=N, MB=68, NB=3, KB=68, 
 * lda=0, ldb=0, ldc=0, mu=2, nu=3, ku=68, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   const float *stM = A + 68;
   const float *stN = B + (((ldb) << 1)+ldb);
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 2 - (((lda) << 6)+((lda) << 2)), incAn = -68;
   #define incBk 68
   const int incBm = -68, incBn = (((ldb) << 1)+ldb);
   #define incAk0 incAk
   const int incBk0 = ((incBk) / 68);
   #define incCm 2
   const int incCn = (((ldc) << 1)+ldc) - 68;
   float *pC0=C, *pC1=pC0+(ldc), *pC2=pC1+(ldc);
   const float *pA0=A;
   const float *pB0=B, *pB1=pB0+(ldb), *pB2=pB1+(ldb);
   register int k;
   register float rA0, rA1;
   register float rB0, rB1, rB2;
   register float m0, m1, m2, m3, m4, m5, m6, m7;
   register float rC0_0, rC1_0, rC0_1, rC1_1, rC0_2, rC1_2;
   do /* M-loop */
   {
/*
 *    Feeble prefetch of C
 */
      rC0_0 = *pC0;
      rC0_1 = *pC1;
      rC0_2 = *pC2;
/*
 *    Start pipeline
 */
      rA0 = *pA0;
      rB0 = *pB0;
      rA1 = pA0[1];
      rB1 = *pB1;
      rB2 = *pB2;
      m0 = rA0 * rB0;
      m1 = rA1 * rB0;
      m2 = rA0 * rB1;
      m3 = rA1 * rB1;
      m4 = rA0 * rB2;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[1];
      rA1 = pA0[1];
      rB1 = pB1[1];
      rB2 = pB2[1];
      m6 = rA0 * rB0;
      m7 = rA1 * rB0;

/*
 *    Completely unrolled K-loop
 */
      rC0_0 = m0;
      m0 = rA0 * rB1;
      rC1_0 = m1;
      m1 = rA1 * rB1;
      rC0_1 = m2;
      m2 = rA0 * rB2;
      rC1_1 = m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[2];
      rA1 = pA0[1];
      rB1 = pB1[2];
      rB2 = pB2[2];
      rC0_2 = m4;
      m4 = rA0 * rB0;
      rC1_2 = m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[3];
      rA1 = pA0[1];
      rB1 = pB1[3];
      rB2 = pB2[3];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[4];
      rA1 = pA0[1];
      rB1 = pB1[4];
      rB2 = pB2[4];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[5];
      rA1 = pA0[1];
      rB1 = pB1[5];
      rB2 = pB2[5];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[6];
      rA1 = pA0[1];
      rB1 = pB1[6];
      rB2 = pB2[6];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[7];
      rA1 = pA0[1];
      rB1 = pB1[7];
      rB2 = pB2[7];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[8];
      rA1 = pA0[1];
      rB1 = pB1[8];
      rB2 = pB2[8];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[9];
      rA1 = pA0[1];
      rB1 = pB1[9];
      rB2 = pB2[9];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[10];
      rA1 = pA0[1];
      rB1 = pB1[10];
      rB2 = pB2[10];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[11];
      rA1 = pA0[1];
      rB1 = pB1[11];
      rB2 = pB2[11];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[12];
      rA1 = pA0[1];
      rB1 = pB1[12];
      rB2 = pB2[12];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[13];
      rA1 = pA0[1];
      rB1 = pB1[13];
      rB2 = pB2[13];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[14];
      rA1 = pA0[1];
      rB1 = pB1[14];
      rB2 = pB2[14];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[15];
      rA1 = pA0[1];
      rB1 = pB1[15];
      rB2 = pB2[15];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[16];
      rA1 = pA0[1];
      rB1 = pB1[16];
      rB2 = pB2[16];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[17];
      rA1 = pA0[1];
      rB1 = pB1[17];
      rB2 = pB2[17];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[18];
      rA1 = pA0[1];
      rB1 = pB1[18];
      rB2 = pB2[18];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[19];
      rA1 = pA0[1];
      rB1 = pB1[19];
      rB2 = pB2[19];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[20];
      rA1 = pA0[1];
      rB1 = pB1[20];
      rB2 = pB2[20];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[21];
      rA1 = pA0[1];
      rB1 = pB1[21];
      rB2 = pB2[21];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[22];
      rA1 = pA0[1];
      rB1 = pB1[22];
      rB2 = pB2[22];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[23];
      rA1 = pA0[1];
      rB1 = pB1[23];
      rB2 = pB2[23];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[24];
      rA1 = pA0[1];
      rB1 = pB1[24];
      rB2 = pB2[24];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[25];
      rA1 = pA0[1];
      rB1 = pB1[25];
      rB2 = pB2[25];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[26];
      rA1 = pA0[1];
      rB1 = pB1[26];
      rB2 = pB2[26];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[27];
      rA1 = pA0[1];
      rB1 = pB1[27];
      rB2 = pB2[27];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[28];
      rA1 = pA0[1];
      rB1 = pB1[28];
      rB2 = pB2[28];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[29];
      rA1 = pA0[1];
      rB1 = pB1[29];
      rB2 = pB2[29];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[30];
      rA1 = pA0[1];
      rB1 = pB1[30];
      rB2 = pB2[30];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[31];
      rA1 = pA0[1];
      rB1 = pB1[31];
      rB2 = pB2[31];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[32];
      rA1 = pA0[1];
      rB1 = pB1[32];
      rB2 = pB2[32];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[33];
      rA1 = pA0[1];
      rB1 = pB1[33];
      rB2 = pB2[33];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[34];
      rA1 = pA0[1];
      rB1 = pB1[34];
      rB2 = pB2[34];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[35];
      rA1 = pA0[1];
      rB1 = pB1[35];
      rB2 = pB2[35];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[36];
      rA1 = pA0[1];
      rB1 = pB1[36];
      rB2 = pB2[36];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[37];
      rA1 = pA0[1];
      rB1 = pB1[37];
      rB2 = pB2[37];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[38];
      rA1 = pA0[1];
      rB1 = pB1[38];
      rB2 = pB2[38];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[39];
      rA1 = pA0[1];
      rB1 = pB1[39];
      rB2 = pB2[39];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[40];
      rA1 = pA0[1];
      rB1 = pB1[40];
      rB2 = pB2[40];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[41];
      rA1 = pA0[1];
      rB1 = pB1[41];
      rB2 = pB2[41];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[42];
      rA1 = pA0[1];
      rB1 = pB1[42];
      rB2 = pB2[42];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[43];
      rA1 = pA0[1];
      rB1 = pB1[43];
      rB2 = pB2[43];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[44];
      rA1 = pA0[1];
      rB1 = pB1[44];
      rB2 = pB2[44];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[45];
      rA1 = pA0[1];
      rB1 = pB1[45];
      rB2 = pB2[45];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[46];
      rA1 = pA0[1];
      rB1 = pB1[46];
      rB2 = pB2[46];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[47];
      rA1 = pA0[1];
      rB1 = pB1[47];
      rB2 = pB2[47];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[48];
      rA1 = pA0[1];
      rB1 = pB1[48];
      rB2 = pB2[48];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[49];
      rA1 = pA0[1];
      rB1 = pB1[49];
      rB2 = pB2[49];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[50];
      rA1 = pA0[1];
      rB1 = pB1[50];
      rB2 = pB2[50];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[51];
      rA1 = pA0[1];
      rB1 = pB1[51];
      rB2 = pB2[51];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[52];
      rA1 = pA0[1];
      rB1 = pB1[52];
      rB2 = pB2[52];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[53];
      rA1 = pA0[1];
      rB1 = pB1[53];
      rB2 = pB2[53];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[54];
      rA1 = pA0[1];
      rB1 = pB1[54];
      rB2 = pB2[54];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[55];
      rA1 = pA0[1];
      rB1 = pB1[55];
      rB2 = pB2[55];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[56];
      rA1 = pA0[1];
      rB1 = pB1[56];
      rB2 = pB2[56];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[57];
      rA1 = pA0[1];
      rB1 = pB1[57];
      rB2 = pB2[57];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[58];
      rA1 = pA0[1];
      rB1 = pB1[58];
      rB2 = pB2[58];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[59];
      rA1 = pA0[1];
      rB1 = pB1[59];
      rB2 = pB2[59];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[60];
      rA1 = pA0[1];
      rB1 = pB1[60];
      rB2 = pB2[60];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[61];
      rA1 = pA0[1];
      rB1 = pB1[61];
      rB2 = pB2[61];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[62];
      rA1 = pA0[1];
      rB1 = pB1[62];
      rB2 = pB2[62];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[63];
      rA1 = pA0[1];
      rB1 = pB1[63];
      rB2 = pB2[63];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[64];
      rA1 = pA0[1];
      rB1 = pB1[64];
      rB2 = pB2[64];
      rC0_2 += m0;
      m0 = rA0 * rB0;
      rC1_2 += m1;
      m1 = rA1 * rB0;
      rC0_0 += m2;
      m2 = rA0 * rB1;
      rC1_0 += m3;
      m3 = rA1 * rB1;
      rC0_1 += m4;
      m4 = rA0 * rB2;
      rC1_1 += m5;
      m5 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[65];
      rA1 = pA0[1];
      rB1 = pB1[65];
      rB2 = pB2[65];
      rC0_2 += m6;
      m6 = rA0 * rB0;
      rC1_2 += m7;
      m7 = rA1 * rB0;
      rC0_0 += m0;
      m0 = rA0 * rB1;
      rC1_0 += m1;
      m1 = rA1 * rB1;
      rC0_1 += m2;
      m2 = rA0 * rB2;
      rC1_1 += m3;
      m3 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[66];
      rA1 = pA0[1];
      rB1 = pB1[66];
      rB2 = pB2[66];
      rC0_2 += m4;
      m4 = rA0 * rB0;
      rC1_2 += m5;
      m5 = rA1 * rB0;
      rC0_0 += m6;
      m6 = rA0 * rB1;
      rC1_0 += m7;
      m7 = rA1 * rB1;
      rC0_1 += m0;
      m0 = rA0 * rB2;
      rC1_1 += m1;
      m1 = rA1 * rB2;
      pA0 += incAk;
      rA0 = *pA0;
      rB0 = pB0[67];
      rA1 = pA0[1];
      rB1 = pB1[67];
      rB2 = pB2[67];
      rC0_2 += m2;
      m2 = rA0 * rB0;
      rC1_2 += m3;
      m3 = rA1 * rB0;
/*
 *    Drain pipe on last iteration of K-loop
 */
      rC0_0 += m4;
      m4 = rA0 * rB1;
      rC1_0 += m5;
      m5 = rA1 * rB1;
      rC0_1 += m6;
      m6 = rA0 * rB2;
      rC1_1 += m7;
      m7 = rA1 * rB2;
      pA0 += incAk;
      rC0_2 += m0;
      rC1_2 += m1;
      rC0_0 += m2;
      rC1_0 += m3;
      rC0_1 += m4;
      rC1_1 += m5;
      rC0_2 += m6;
      rC1_2 += m7;
      pB0 += incBk;
      pB1 += incBk;
      pB2 += incBk;
      rB0 = alpha;
      rC0_0 *= rB0;
      rC0_1 *= rB0;
      rC0_2 *= rB0;
      rC1_0 *= rB0;
      rC1_1 *= rB0;
      rC1_2 *= rB0;
      *pC0 = rC0_0;
      pC0[1] = rC1_0;
      *pC1 = rC0_1;
      pC1[1] = rC1_1;
      *pC2 = rC0_2;
      pC2[1] = rC1_2;
      pC0 += incCm;
      pC1 += incCm;
      pC2 += incCm;
      pA0 += incAm;
      pB0 += incBm;
      pB1 += incBm;
      pB2 += incBm;
   }
   while(pA0 != stM);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
void ATL_sJIK68x68x68NN0x0x0_aX_b0
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=N, MB=68, NB=68, KB=68, 
 * lda=0, ldb=0, ldc=0, mu=2, nu=5, ku=68, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.2)
 */
{
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + 68;
   const float *stN = B + (((ldb) << 6)+ldb);
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 2 - (((lda) << 6)+((lda) << 2)), incAn = -68;
   #define incBk 68
   const int incBm = -68, incBn = (((ldb) << 2)+ldb);
   #define incAk0 incAk
   const int incBk0 = ((incBk) / 68);
   #define incCm 2
   const int incCn = (((ldc) << 2)+ldc) - 68;
   float *pC0=C, *pC1=pC0+(ldc), *pC2=pC1+(ldc), *pC3=pC2+(ldc), *pC4=pC3+(ldc);
   const float *pA0=A;
   const float *pB0=B, *pB1=pB0+(ldb), *pB2=pB1+(ldb), *pB3=pB2+(ldb), *pB4=pB3+(ldb);
   register int k;
   register float rA0, rA1;
   register float rB0, rB1, rB2, rB3, rB4;
   register float m0, m1, m2, m3, m4, m5, m6, m7;
   register float rC0_0, rC1_0, rC0_1, rC1_1, rC0_2, rC1_2, rC0_3, rC1_3, rC0_4, rC1_4;
   do /* N-loop */
   {
      do /* M-loop */
      {
/*
 *       Feeble prefetch of C
 */
         rC0_0 = *pC0;
         rC0_1 = *pC1;
         rC0_2 = *pC2;
         rC0_3 = *pC3;
         rC0_4 = *pC4;
/*
 *       Start pipeline
 */
         rA0 = *pA0;
         rB0 = *pB0;
         rA1 = pA0[1];
         rB1 = *pB1;
         rB2 = *pB2;
         rB3 = *pB3;
         m0 = rA0 * rB0;
         rB4 = *pB4;
         m1 = rA1 * rB0;
         m2 = rA0 * rB1;
         m3 = rA1 * rB1;
         m4 = rA0 * rB2;
         m5 = rA1 * rB2;
         m6 = rA0 * rB3;
         m7 = rA1 * rB3;

/*
 *       Completely unrolled K-loop
 */
         rC0_0 = m0;
         m0 = rA0 * rB4;
         rC1_0 = m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[1];
         rA1 = pA0[1];
         rB1 = pB1[1];
         rB2 = pB2[1];
         rB3 = pB3[1];
         rB4 = pB4[1];
         rC0_1 = m2;
         m2 = rA0 * rB0;
         rC1_1 = m3;
         m3 = rA1 * rB0;
         rC0_2 = m4;
         m4 = rA0 * rB1;
         rC1_2 = m5;
         m5 = rA1 * rB1;
         rC0_3 = m6;
         m6 = rA0 * rB2;
         rC1_3 = m7;
         m7 = rA1 * rB2;
         rC0_4 = m0;
         m0 = rA0 * rB3;
         rC1_4 = m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[2];
         rA1 = pA0[1];
         rB1 = pB1[2];
         rB2 = pB2[2];
         rB3 = pB3[2];
         rB4 = pB4[2];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[3];
         rA1 = pA0[1];
         rB1 = pB1[3];
         rB2 = pB2[3];
         rB3 = pB3[3];
         rB4 = pB4[3];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[4];
         rA1 = pA0[1];
         rB1 = pB1[4];
         rB2 = pB2[4];
         rB3 = pB3[4];
         rB4 = pB4[4];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[5];
         rA1 = pA0[1];
         rB1 = pB1[5];
         rB2 = pB2[5];
         rB3 = pB3[5];
         rB4 = pB4[5];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[6];
         rA1 = pA0[1];
         rB1 = pB1[6];
         rB2 = pB2[6];
         rB3 = pB3[6];
         rB4 = pB4[6];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[7];
         rA1 = pA0[1];
         rB1 = pB1[7];
         rB2 = pB2[7];
         rB3 = pB3[7];
         rB4 = pB4[7];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[8];
         rA1 = pA0[1];
         rB1 = pB1[8];
         rB2 = pB2[8];
         rB3 = pB3[8];
         rB4 = pB4[8];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[9];
         rA1 = pA0[1];
         rB1 = pB1[9];
         rB2 = pB2[9];
         rB3 = pB3[9];
         rB4 = pB4[9];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[10];
         rA1 = pA0[1];
         rB1 = pB1[10];
         rB2 = pB2[10];
         rB3 = pB3[10];
         rB4 = pB4[10];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[11];
         rA1 = pA0[1];
         rB1 = pB1[11];
         rB2 = pB2[11];
         rB3 = pB3[11];
         rB4 = pB4[11];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[12];
         rA1 = pA0[1];
         rB1 = pB1[12];
         rB2 = pB2[12];
         rB3 = pB3[12];
         rB4 = pB4[12];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[13];
         rA1 = pA0[1];
         rB1 = pB1[13];
         rB2 = pB2[13];
         rB3 = pB3[13];
         rB4 = pB4[13];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[14];
         rA1 = pA0[1];
         rB1 = pB1[14];
         rB2 = pB2[14];
         rB3 = pB3[14];
         rB4 = pB4[14];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[15];
         rA1 = pA0[1];
         rB1 = pB1[15];
         rB2 = pB2[15];
         rB3 = pB3[15];
         rB4 = pB4[15];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[16];
         rA1 = pA0[1];
         rB1 = pB1[16];
         rB2 = pB2[16];
         rB3 = pB3[16];
         rB4 = pB4[16];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[17];
         rA1 = pA0[1];
         rB1 = pB1[17];
         rB2 = pB2[17];
         rB3 = pB3[17];
         rB4 = pB4[17];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[18];
         rA1 = pA0[1];
         rB1 = pB1[18];
         rB2 = pB2[18];
         rB3 = pB3[18];
         rB4 = pB4[18];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[19];
         rA1 = pA0[1];
         rB1 = pB1[19];
         rB2 = pB2[19];
         rB3 = pB3[19];
         rB4 = pB4[19];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[20];
         rA1 = pA0[1];
         rB1 = pB1[20];
         rB2 = pB2[20];
         rB3 = pB3[20];
         rB4 = pB4[20];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[21];
         rA1 = pA0[1];
         rB1 = pB1[21];
         rB2 = pB2[21];
         rB3 = pB3[21];
         rB4 = pB4[21];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[22];
         rA1 = pA0[1];
         rB1 = pB1[22];
         rB2 = pB2[22];
         rB3 = pB3[22];
         rB4 = pB4[22];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[23];
         rA1 = pA0[1];
         rB1 = pB1[23];
         rB2 = pB2[23];
         rB3 = pB3[23];
         rB4 = pB4[23];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[24];
         rA1 = pA0[1];
         rB1 = pB1[24];
         rB2 = pB2[24];
         rB3 = pB3[24];
         rB4 = pB4[24];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[25];
         rA1 = pA0[1];
         rB1 = pB1[25];
         rB2 = pB2[25];
         rB3 = pB3[25];
         rB4 = pB4[25];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[26];
         rA1 = pA0[1];
         rB1 = pB1[26];
         rB2 = pB2[26];
         rB3 = pB3[26];
         rB4 = pB4[26];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[27];
         rA1 = pA0[1];
         rB1 = pB1[27];
         rB2 = pB2[27];
         rB3 = pB3[27];
         rB4 = pB4[27];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[28];
         rA1 = pA0[1];
         rB1 = pB1[28];
         rB2 = pB2[28];
         rB3 = pB3[28];
         rB4 = pB4[28];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[29];
         rA1 = pA0[1];
         rB1 = pB1[29];
         rB2 = pB2[29];
         rB3 = pB3[29];
         rB4 = pB4[29];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[30];
         rA1 = pA0[1];
         rB1 = pB1[30];
         rB2 = pB2[30];
         rB3 = pB3[30];
         rB4 = pB4[30];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[31];
         rA1 = pA0[1];
         rB1 = pB1[31];
         rB2 = pB2[31];
         rB3 = pB3[31];
         rB4 = pB4[31];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[32];
         rA1 = pA0[1];
         rB1 = pB1[32];
         rB2 = pB2[32];
         rB3 = pB3[32];
         rB4 = pB4[32];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[33];
         rA1 = pA0[1];
         rB1 = pB1[33];
         rB2 = pB2[33];
         rB3 = pB3[33];
         rB4 = pB4[33];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[34];
         rA1 = pA0[1];
         rB1 = pB1[34];
         rB2 = pB2[34];
         rB3 = pB3[34];
         rB4 = pB4[34];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[35];
         rA1 = pA0[1];
         rB1 = pB1[35];
         rB2 = pB2[35];
         rB3 = pB3[35];
         rB4 = pB4[35];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[36];
         rA1 = pA0[1];
         rB1 = pB1[36];
         rB2 = pB2[36];
         rB3 = pB3[36];
         rB4 = pB4[36];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[37];
         rA1 = pA0[1];
         rB1 = pB1[37];
         rB2 = pB2[37];
         rB3 = pB3[37];
         rB4 = pB4[37];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[38];
         rA1 = pA0[1];
         rB1 = pB1[38];
         rB2 = pB2[38];
         rB3 = pB3[38];
         rB4 = pB4[38];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[39];
         rA1 = pA0[1];
         rB1 = pB1[39];
         rB2 = pB2[39];
         rB3 = pB3[39];
         rB4 = pB4[39];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[40];
         rA1 = pA0[1];
         rB1 = pB1[40];
         rB2 = pB2[40];
         rB3 = pB3[40];
         rB4 = pB4[40];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[41];
         rA1 = pA0[1];
         rB1 = pB1[41];
         rB2 = pB2[41];
         rB3 = pB3[41];
         rB4 = pB4[41];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[42];
         rA1 = pA0[1];
         rB1 = pB1[42];
         rB2 = pB2[42];
         rB3 = pB3[42];
         rB4 = pB4[42];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[43];
         rA1 = pA0[1];
         rB1 = pB1[43];
         rB2 = pB2[43];
         rB3 = pB3[43];
         rB4 = pB4[43];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[44];
         rA1 = pA0[1];
         rB1 = pB1[44];
         rB2 = pB2[44];
         rB3 = pB3[44];
         rB4 = pB4[44];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[45];
         rA1 = pA0[1];
         rB1 = pB1[45];
         rB2 = pB2[45];
         rB3 = pB3[45];
         rB4 = pB4[45];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[46];
         rA1 = pA0[1];
         rB1 = pB1[46];
         rB2 = pB2[46];
         rB3 = pB3[46];
         rB4 = pB4[46];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[47];
         rA1 = pA0[1];
         rB1 = pB1[47];
         rB2 = pB2[47];
         rB3 = pB3[47];
         rB4 = pB4[47];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[48];
         rA1 = pA0[1];
         rB1 = pB1[48];
         rB2 = pB2[48];
         rB3 = pB3[48];
         rB4 = pB4[48];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[49];
         rA1 = pA0[1];
         rB1 = pB1[49];
         rB2 = pB2[49];
         rB3 = pB3[49];
         rB4 = pB4[49];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[50];
         rA1 = pA0[1];
         rB1 = pB1[50];
         rB2 = pB2[50];
         rB3 = pB3[50];
         rB4 = pB4[50];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[51];
         rA1 = pA0[1];
         rB1 = pB1[51];
         rB2 = pB2[51];
         rB3 = pB3[51];
         rB4 = pB4[51];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[52];
         rA1 = pA0[1];
         rB1 = pB1[52];
         rB2 = pB2[52];
         rB3 = pB3[52];
         rB4 = pB4[52];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[53];
         rA1 = pA0[1];
         rB1 = pB1[53];
         rB2 = pB2[53];
         rB3 = pB3[53];
         rB4 = pB4[53];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[54];
         rA1 = pA0[1];
         rB1 = pB1[54];
         rB2 = pB2[54];
         rB3 = pB3[54];
         rB4 = pB4[54];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[55];
         rA1 = pA0[1];
         rB1 = pB1[55];
         rB2 = pB2[55];
         rB3 = pB3[55];
         rB4 = pB4[55];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[56];
         rA1 = pA0[1];
         rB1 = pB1[56];
         rB2 = pB2[56];
         rB3 = pB3[56];
         rB4 = pB4[56];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[57];
         rA1 = pA0[1];
         rB1 = pB1[57];
         rB2 = pB2[57];
         rB3 = pB3[57];
         rB4 = pB4[57];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[58];
         rA1 = pA0[1];
         rB1 = pB1[58];
         rB2 = pB2[58];
         rB3 = pB3[58];
         rB4 = pB4[58];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[59];
         rA1 = pA0[1];
         rB1 = pB1[59];
         rB2 = pB2[59];
         rB3 = pB3[59];
         rB4 = pB4[59];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[60];
         rA1 = pA0[1];
         rB1 = pB1[60];
         rB2 = pB2[60];
         rB3 = pB3[60];
         rB4 = pB4[60];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[61];
         rA1 = pA0[1];
         rB1 = pB1[61];
         rB2 = pB2[61];
         rB3 = pB3[61];
         rB4 = pB4[61];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[62];
         rA1 = pA0[1];
         rB1 = pB1[62];
         rB2 = pB2[62];
         rB3 = pB3[62];
         rB4 = pB4[62];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[63];
         rA1 = pA0[1];
         rB1 = pB1[63];
         rB2 = pB2[63];
         rB3 = pB3[63];
         rB4 = pB4[63];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[64];
         rA1 = pA0[1];
         rB1 = pB1[64];
         rB2 = pB2[64];
         rB3 = pB3[64];
         rB4 = pB4[64];
         rC0_1 += m0;
         m0 = rA0 * rB0;
         rC1_1 += m1;
         m1 = rA1 * rB0;
         rC0_2 += m2;
         m2 = rA0 * rB1;
         rC1_2 += m3;
         m3 = rA1 * rB1;
         rC0_3 += m4;
         m4 = rA0 * rB2;
         rC1_3 += m5;
         m5 = rA1 * rB2;
         rC0_4 += m6;
         m6 = rA0 * rB3;
         rC1_4 += m7;
         m7 = rA1 * rB3;
         rC0_0 += m0;
         m0 = rA0 * rB4;
         rC1_0 += m1;
         m1 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[65];
         rA1 = pA0[1];
         rB1 = pB1[65];
         rB2 = pB2[65];
         rB3 = pB3[65];
         rB4 = pB4[65];
         rC0_1 += m2;
         m2 = rA0 * rB0;
         rC1_1 += m3;
         m3 = rA1 * rB0;
         rC0_2 += m4;
         m4 = rA0 * rB1;
         rC1_2 += m5;
         m5 = rA1 * rB1;
         rC0_3 += m6;
         m6 = rA0 * rB2;
         rC1_3 += m7;
         m7 = rA1 * rB2;
         rC0_4 += m0;
         m0 = rA0 * rB3;
         rC1_4 += m1;
         m1 = rA1 * rB3;
         rC0_0 += m2;
         m2 = rA0 * rB4;
         rC1_0 += m3;
         m3 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[66];
         rA1 = pA0[1];
         rB1 = pB1[66];
         rB2 = pB2[66];
         rB3 = pB3[66];
         rB4 = pB4[66];
         rC0_1 += m4;
         m4 = rA0 * rB0;
         rC1_1 += m5;
         m5 = rA1 * rB0;
         rC0_2 += m6;
         m6 = rA0 * rB1;
         rC1_2 += m7;
         m7 = rA1 * rB1;
         rC0_3 += m0;
         m0 = rA0 * rB2;
         rC1_3 += m1;
         m1 = rA1 * rB2;
         rC0_4 += m2;
         m2 = rA0 * rB3;
         rC1_4 += m3;
         m3 = rA1 * rB3;
         rC0_0 += m4;
         m4 = rA0 * rB4;
         rC1_0 += m5;
         m5 = rA1 * rB4;
         pA0 += incAk;
         rA0 = *pA0;
         rB0 = pB0[67];
         rA1 = pA0[1];
         rB1 = pB1[67];
         rB2 = pB2[67];
         rB3 = pB3[67];
         rB4 = pB4[67];
         rC0_1 += m6;
         m6 = rA0 * rB0;
         rC1_1 += m7;
         m7 = rA1 * rB0;
         rC0_2 += m0;
         m0 = rA0 * rB1;
         rC1_2 += m1;
         m1 = rA1 * rB1;
         rC0_3 += m2;
         m2 = rA0 * rB2;
         rC1_3 += m3;
         m3 = rA1 * rB2;
         rC0_4 += m4;
         m4 = rA0 * rB3;
         rC1_4 += m5;
         m5 = rA1 * rB3;
/*
 *       Drain pipe on last iteration of K-loop
 */
         rC0_0 += m6;
         m6 = rA0 * rB4;
         rC1_0 += m7;
         m7 = rA1 * rB4;
         pA0 += incAk;
         rC0_1 += m0;
         rC1_1 += m1;
         rC0_2 += m2;
         rC1_2 += m3;
         rC0_3 += m4;
         rC1_3 += m5;
         rC0_4 += m6;
         rC1_4 += m7;
         pB0 += incBk;
         pB1 += incBk;
         pB2 += incBk;
         pB3 += incBk;
         pB4 += incBk;
         rB0 = alpha;
         rC0_0 *= rB0;
         rC0_1 *= rB0;
         rC0_2 *= rB0;
         rC0_3 *= rB0;
         rC0_4 *= rB0;
         rC1_0 *= rB0;
         rC1_1 *= rB0;
         rC1_2 *= rB0;
         rC1_3 *= rB0;
         rC1_4 *= rB0;
         *pC0 = rC0_0;
         pC0[1] = rC1_0;
         *pC1 = rC0_1;
         pC1[1] = rC1_1;
         *pC2 = rC0_2;
         pC2[1] = rC1_2;
         *pC3 = rC0_3;
         pC3[1] = rC1_3;
         *pC4 = rC0_4;
         pC4[1] = rC1_4;
         pC0 += incCm;
         pC1 += incCm;
         pC2 += incCm;
         pC3 += incCm;
         pC4 += incCm;
         pA0 += incAm;
         pB0 += incBm;
         pB1 += incBm;
         pB2 += incBm;
         pB3 += incBm;
         pB4 += incBm;
      }
      while(pA0 != stM);
      pC0 += incCn;
      pC1 += incCn;
      pC2 += incCn;
      pC3 += incCn;
      pC4 += incCn;
      pA0 += incAn;
      pB0 += incBn;
      pB1 += incBn;
      pB2 += incBn;
      pB3 += incBn;
      pB4 += incBn;
   }
   while(pB0 != stN);
   ATL_sJIK68x3x68NN2x3x68_aX_b0(68, 3, 68, alpha, ca, lda, cb + (((ldb) << 6)+ldb), ldb, beta, cc + (((ldc) << 6)+ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
