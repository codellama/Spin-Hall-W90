  ...
  !============================================================!
   ...
   integer            :: s_adpt_counter_list(nfermi)     ! adaptive kmesh counter
    ...
    if(eval_ahc) then
       ...
       call get_SS_R                    ! Also calling SS_R here
       imf_list=0.0_dp
       adpt_counter_list=0
       sbc_list=0.0_dp                  ! SHC
...
!                if(berry_curv_unit=='bohr2') vdum=vdum/bohr**2
    ...
    if(on_root) then
          ...
          if(nfermi==1) then
             if(wanint_kpoint_file) then
                write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                     ' Points triggering refinement AHC: ',&
                     adpt_counter_list(1),'(',&
                     100*real(adpt_counter_list(1),dp)&
                     /sum(num_int_kpts_on_node),'%)'
                write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                     ' Points triggering refinement SHC: ',&
                     s_adpt_counter_list(1),'(',&
                     100*real(s_adpt_counter_list(1),dp)&
                     /sum(num_int_kpts_on_node),'%)'
             else
                write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                     ' Points triggering refinement AHC: ',&
                ...
                write(stdout,'(1x,a30,i5,a,f5.2,a)')&
                     ' Points triggering refinement SHC: ',&
                     s_adpt_counter_list(1),'(',&
                     100*real(s_adpt_counter_list(1),dp)/product(berry_kmesh),'%)'
             endif
          endif
       else
       ...
       if(eval_ahc) then                ! Output for SHC
          ...
          ! points in our BZ mesh in Ang^2 and needs an extra factor of hbar/2 
          !
          ...
          ! involves an extra factor of e/hbar to get a conductivity in units of 1/(Ohm-length),
          ! So that SHC values are quoted in (hbar/e)(S/cm) units
          !
          ! Also, it looks like traditionally a negative sign is applied
          !
          ! Assuming Berry curvature was in units of Ang^-2
          ! with 'V_c' in Angstroms^3, and 'e', 'hbar' in SI units
          !
          ! So we need to multiply by -1*(hbar/2)*(-e/hbar)*(1/V_c)*(e/hbar)
          !
          ! So we multiply sbc_list by fac where:
          ! ======================================
          ! fac = e^2/(2*hbar.V_c*10^-8) 
          ! ======================================
          ...
          ! --------------------------------------------------------------------
          !
          fac=1.0e8_dp*elem_charge_SI**2/(2*hbar_SI*cell_volume)
          ...
          do if=1,nfermi
             if(nfermi>1) write(file_unit,'((F12.6,1x),3(E20.7))')&
  ...
  !=======================================================================!
    ...
    ! get_HH_R and get_AA_R & get_SS_R have already been called by berry_main if(eval_ahc) 
   ...
   inquire(file=file_name, exist=exist)         !RF
   if (exist) then                              !RF
    open(file_unit,file=file_name, status="old", position="append", action="write")     !RF
   else         !RF
    open(file_unit, file=file_name, status="new", action="write")       !RF
   end if                               !RF
...
!    do i=1,3                            ! RF
!      write(file_unit,'(/,1x,a,I2)') 'Re, Im of AA(nw,nw,3),component',i      !RF
!        do n=1,num_wann                       ! RF
!          write(file_unit,'(/)')  !RF
!          write(file_unit,'(F12.6,F12.6)') real(AA(n,:,i)), aimag(AA(n,:,i)) !RF
!        enddo                   ! RF
!      write(file_unit,'(/,1x,a,I2)') 'Re, Im of delHH_bar(nw,nw,3),component',i      !RF
!        do n=1,num_wann         ! RF
!          write(file_unit,'(/)')  !RF
!          write(file_unit,'(F12.6,F12.6)') real(delHH_bar(n,:,i)),& 
!                                           aimag(delHH_bar(n,:,i))     !RF
!        enddo                   ! RF
!    enddo                       ! RF
...
!   write(file_unit,'(/)')  !RF
        do m=1,num_wann
           vel(n,m,i) = delHH_bar(n,m,i) - (cmplx_i)&
                        *(eig(m)-eig(n))*(AA(n,m,i))
!   write(file_unit,'(E13.5,E13.5)') real(vel(n,m,i)),aimag(vel(n,m,i)) !RF
        enddo
      enddo
   enddo
...
!  !Writing out SS_R
!    do i=1,3                            ! RF
!      write(file_unit,'(/,1x,a,I2)') 'SS_R(nw,nw,nrpts,3),Pauli matrix, component',i      !RF
!        do n=1,num_wann                       ! RF
!    write(file_unit,'(/)')  !RF
!    write(file_unit,'(2(F12.6,1x))') SS_R(n,:,:,i) !RF
!        enddo                   ! RF
!    enddo                       ! RF
...
!        do n=1,num_wann                       ! RF
!    write(file_unit,'(/)')  !RF
!    write(file_unit,'(F12.6,F12.6)') real(SS(n,:,i)), aimag(SS(n,:,i)) !RF
!        enddo                   ! RF
!    enddo                       ! RF

   ! However SS_R stores the Pauli marix not spin matrix so we need a factor
   ! of hbar/2 <-- Not doing this
!   SS(:,:,:) = SS(:,:,:)*hbar_SI/2
...
!   write(file_unit,'(/)')  !RF
          do m=1,num_wann
            ...
            enddo
!   write(file_unit,'(E13.5,E13.5)') real(JS(n,m,alpha,s)),&
!                                    aimag(JS(n,m,alpha,s)) !RF
          enddo
        enddo
     enddo
   enddo

 !     write(file_unit,'(/,a)') 'JS*vel, JS*vel/(E)**2'
     ...
     do alpha=1,3 
        do beta=1,3
          do s=1,3
             do m=1,num_wann
                if(m/=n) then
   SO_temp = -2*aimag((JS(n,m,alpha,s)*vel(m,n,beta))/(eig(m)-eig(n))**2)
                     SOmega_N(n,alpha,beta,s) = &
                     SOmega_N(n,alpha,beta,s) + SO_temp
! write(file_unit,'(E20.5,E20.5)') aimag(JS(n,m,alpha,s)*vel(m,n,beta)),SO_temp  !RF
                endif
             enddo  
          enddo
        enddo
...
!                                alpha,beta,s      ! RF
!      write(file_unit,'(E20.5)') SOmega_N(:,alpha,beta,s) !RF
!        enddo
!      enddo                       ! RF
!    enddo
...
!      write(file_unit,'(/,a,(F12.6))') 'occupation', occ(:) !RF
!      write(file_unit,'(/,a,(F12.6))') 'eigenvals', eig(:) !RF
      do n=1,num_wann 
 ...
 ! Writing Spin BC
    do i=1,3                            
      write(file_unit,'(/,1x,a,I2)') 'SBC_klist, component:',i  !RF
      write(file_unit,'(E20.5,1x)') sbc_k_list(i,:) !RF
    enddo 
