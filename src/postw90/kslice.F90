...
module w90_kslice
  ...
  ! 
  !  - The spin Berry curvature summed over occupied bands 
  !
  ...
  subroutine k_slice
    ...
    real(kind=dp)     :: bvec(3,3),yvec(3),zvec(3),b1mod,b2mod,ymod,cosb1b2,&
                         ...
                         sbc_k_list(3,nfermi), spn_curv(3)   !SBC
    ...
    if(plot_curv) call get_SS_R                 ! SBC
    if(plot_morb) then
       call get_BB_R
       call get_CC_R
    endif
...
!       if (abs(cosb1b2)<eps8 .and. b1mod==b2mod) then
       ...
       if(plot_curv) then
          ...
          spn_zdataunit=io_file_unit()                  !SBC
          filename_sbc=trim(seedname)//'-kslice-spn_curv.dat'   !SBC
          write(stdout,'(/,3x,a)') filename_sbc         !SBC
          open(spn_zdataunit,file=filename_sbc,form='formatted')        !SBC
...
!                   Delta_E=Delta_E*sqrt(2.0_dp) ! optimize this factor
          ...
          if(plot_curv) then
             ...
             call get_sbc_k_list(kpt,sbc_k_list)        !SBC
             spn_curv(1)=sbc_k_list(1,1)        !SBC
             spn_curv(2)=sbc_k_list(2,1)        !SBC
             spn_curv(3)=sbc_k_list(3,1)        !SBC
!             if(berry_curv_unit=='bohr2') curv=curv/bohr**2   
             write(spn_zdataunit,'(3E16.8)') -spn_curv(:)        !SBC
             ! Printing negative spin berry curvature
          endif
       ...
       if(heatmap) then
          ...
          close(zdataunit)
          if(plot_curv) then
             write(spn_zdataunit,*) ' '    !SBC
             close(spn_zdataunit)          !SBC
          endif 
       endif
       if(plot_fermi_lines) then
          if(fermi_lines_color) then
       ...
       if(heatmap) then
        ...
        ! Python scripts for SBC
       if(heatmap) then
          !
          ! python script for curvature SBC heatmaps [+ black Fermi lines]
          !
          do i=1,3

             scriptunit=io_file_unit()
             if(plot_curv .and. .not.plot_fermi_lines) then
                filename_sbc=trim(seedname)//'-kslice-spn_curv_'//achar(119+i)//'.py'
                write(stdout,'(/,3x,a)') filename_sbc
                open(scriptunit,file=filename_sbc,form='formatted')
             elseif(plot_curv .and. plot_fermi_lines) then
                filename_sbc=trim(seedname)//'-kslice-spn_curv_'//achar(119+i)//&
                     '+fermi_lines.py'
                write(stdout,'(/,3x,a)') filename_sbc
                open(scriptunit,file=filename_sbc,form='formatted')
             endif
             ...
             if(plot_curv) then
                write(scriptunit,'(a)') ' '
                write(scriptunit,'(a)') "outfile = '"//trim(seedname)//&
               "-kslice-spn_curv_"//achar(119+i)//".pdf'"
                write(scriptunit,'(a)') ' '
                write(scriptunit,'(a)')&
                     "val = np.loadtxt('"//trim(seedname)//&
                     "-kslice-spn_curv.dat', usecols=("//achar(47+i)//",))"
                write(scriptunit,'(a)') ' '
                write(scriptunit,'(a)')&
                     'val_log=np.array([np.log10(abs(elem))*np.sign(elem) &
                 &if abs(elem)>10 else elem/10.0 for elem in val])'
